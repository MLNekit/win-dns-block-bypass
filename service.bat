...
title AdGuard Home Service Manager v%VERSION%
mode con cols=92 lines=42 >nul 2>&1
color 0B
...
echo.
echo   ╔══════════════════════════════════════════════════════════════════════════════╗
echo   ║                    ADGUARD HOME SERVICE MANAGER                             ║
echo   ║                              v%VERSION%                                     ║
echo   ╚══════════════════════════════════════════════════════════════════════════════╝
echo.
echo   ┌─ СОСТОЯНИЕ ────────────────────────────────────────────────────────────────┐
echo   │  %SVC_ICON% Служба : %SVC_STATUS%   %DNS_ICON% DNS : %DNS_PORT_STATUS% │
powershell -NoProfile -Command "$p=$env:AGH_YAML; $addr=''; $sec=''; if(Test-Path -LiteralPath $p){foreach($l in Get-Content -LiteralPath $p){if($l -match '^http:\s*$'){$sec='http';continue}; if($l -match '^tls:\s*$'){$sec='';continue}; if($l -match '^\S' -and $l -notmatch '^http:\s*$'){$sec=''}; if($sec -eq 'http' -and $l -match '^\s{2}address:\s*(.+)$'){$addr=$matches[1].Trim().Trim([char]34);break}}}; if($addr){Write-Host (('   │  [i] Веб-панель: http://127.0.0.1:'+($addr -replace '.*:', ''))+' ') -ForegroundColor Cyan} else {Write-Host '   │  [i] Веб-панель: порт читается из AdGuardHome.yaml' -ForegroundColor DarkGray}"
echo   └───────────────────────────────────────────────────────────────────────────┘
echo.
echo   ┌─ DNS 127.0.0.1 ────────────────────────────────────────────────────────────┐
...
echo   ─────────────────────────────────────────────────────────────────────────────
set "MENU_CHOICE="
set /p "MENU_CHOICE=   Ваш выбор [0-9]: "
...
:: ========================================================
:: УСТАНОВКА СЛУЖБЫ + СРАЗУ ВЫБОР DNS
:: ========================================================
:service_install
...
if not exist "%AGH_YAML%" goto install_no_yaml
...
:install_no_yaml
echo   [X] src\AdGuardHome.yaml не найден.
echo.
pause
goto menu
...
:: ========================================================
:: WEB DASHBOARD - ПОРТ ИЗ AdGuardHome.yaml
:: ========================================================
:open_web
call :action_header "ВЕБ-ПАНЕЛЬ ADGUARD HOME" "Адрес и порт читаются напрямую из AdGuardHome.yaml."
if not exist "%AGH_YAML%" goto web_no_yaml
